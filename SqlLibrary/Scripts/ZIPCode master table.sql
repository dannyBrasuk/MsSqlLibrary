...
/*
	...
	Historical table of ZIP Codes.  Will be used to determine which ZIP Codes have changed in terms of delviery area over time.

	Draft: beta
*/

IF OBJECT_ID('appData.PostalCodeUsaHistoricalMaster') IS NOT NULL AND NOT EXISTS( SELECT TOP 1 [PostalCodeUsaHistoricalMaster_pk] FROM appData.PostalCodeUsaHistoricalMaster)
	DROP TABLE appData.PostalCodeUsaHistoricalMaster ;
GO
...
--spatial on boundary -- assume auto grid for now.   (no boundary box with geography, so state filter might not help)
